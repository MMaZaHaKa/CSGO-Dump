...
public:
	...
	Vector3D(void);
	Vector3D(vec_t X, vec_t Y);
	Vector3D(vec_t* clr);

	Vector3D::Vector3D(const Vector3D &vOther)
	{
	...
	bool WithinAABox(Vector3D const &boxmin, Vector3D const &boxmax);
	...
	vec_t DistToSqr(const Vector3D &vOther) const
	{
		Vector3D delta;
	...
	Vector3D  ProjectOnto(const Vector3D& onto);
...
{
	Vertex_t2() {}
	Vertex_t2(const Vector3D &pos, const Vector3D &coord = Vector3D(0, 0))
	{
		m_Position = pos;
		m_TexCoord = coord;
	}
	void Init(const Vector3D &pos, const Vector3D &coord = Vector3D(0, 0))
	{
		m_Position = pos;
		m_TexCoord = coord;
	}
...
public:
	...
	virtual void           DrawTexturedLine(const Vertex_t2 &a, const Vertex_t2 &b) = 0;
	virtual void           DrawOutlinedCircle(int x, int y, int radius, int segments) = 0;
	virtual void           DrawTexturedPolyLine(const Vertex_t2 *p, int n) = 0;
	virtual void           DrawTexturedSubRect(int x0, int y0, int x1, int y1, float texs0, float text0, float texs1, float text1) = 0;
	virtual void           DrawTexturedPolygon(int n, Vertex_t2 *pVertice, bool bClipVertices = true) = 0;
...
class CLabel : public CObject
{
	...
	bool m_centered;
public:
	CLabel(std::wstring text = L"", vgui::HFont font = 0, Color color = Color::Purple(), bool centered = false);
	void Paint(int t_x = 0, int t_y = 0, float* controlheight = nullptr);
};
